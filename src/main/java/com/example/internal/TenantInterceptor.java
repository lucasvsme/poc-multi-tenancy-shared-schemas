...
public final class TenantInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws IOException {
        ...
        if (!tenants.getTenantNames().contains(xTenantId)) {
    ...
    private boolean selectTenantAndContinue(String xTenantId) {
        TenantReference.set(tenants.getByTenantName(xTenantId));
        LOGGER.debug("Handling request for tenant {}", TenantReference.get());
        return true;
    }
}
